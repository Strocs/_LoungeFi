...
export const startLoadData = async (uid = '') => {
  ...
  for (const doc of groupDocs.docs) {
    const tasksDocs = await getDocs(collection(FirebaseDB, `${uid}/${doc.id}/tasks`))

    const tasks = []

    tasksDocs.forEach((task) => tasks.push({ dbId: task.id, ...task.data() }))
...
export const startReorderTasks = async (uid = '', newOrder, group) => {
  ...
  newOrder.forEach((taskId, index) => {
    const taskToUpdate = tasksToUpdate.find((task) => task.dbId === taskId.dbId)
    if (taskToUpdate) {
      taskToUpdate.data.order = index + 1
      const taskDocRef = doc(FirebaseDB, tasksRef.path, taskToUpdate.dbId)
...
export const startUpdateTask = async (uid = '', task) => {
  ...
  delete taskToFireStore.dbId

  const docRef = doc(FirebaseDB, `${uid}/${task.group}/tasks/${task.dbId}`)
