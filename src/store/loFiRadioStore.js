...
export const useLoFiRadioStore = create((set, get) => ({
  ...
  setActiveRadio: (radioIndex = 0) => {
    set(state => ({
      activeRadio: { index: radioIndex, ...state.loFiRadioList[radioIndex] }
    }))
  },
  onPlayOrPause: () => {
  ...
  playNext: () => {
    if (get().activeRadio.index >= get().loFiRadioList.length - 1)
      return get().setActiveRadio(0)
    get().setActiveRadio(get().activeRadio.index + 1)
  },
  playLast: () => {
    if (get().activeRadio.index <= 0)
      return get().setActiveRadio(get().loFiRadioList.length - 1)
    get().setActiveRadio(get().activeRadio.index - 1)
  }
}))
